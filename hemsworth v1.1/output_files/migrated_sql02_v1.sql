...
SELECT
    ...
    bl.leg_competition_name_reporting AS competition_name,
    ...
    CASE
        WHEN bl.bet_last_settled_local_ts >= current_date() - INTERVAL 14 DAYS THEN CAST(bl.leg_event_id_ramp AS VARCHAR(255))
    END AS event_id,
    CASE
        WHEN bl.bet_last_settled_local_ts >= current_date() - INTERVAL 14 DAYS THEN CAST(bl.leg_market_id_openbet AS VARCHAR(255))
    END AS market_id,
    CASE
        WHEN bl.bet_last_settled_local_ts >= current_date() - INTERVAL 14 DAYS THEN CAST(bl.leg_selection_id_openbet AS VARCHAR(255))
    END AS selection_id,
    
    -- In-play indicator (simplified)
    CASE
        WHEN bl.is_event_in_play = TRUE THEN 'In-Play'
        ELSE 'Pre-Match'
    END AS in_play_yn,
    ...
    CASE
        ...
        WHEN bl.leg_price_type_code = 'sgm' AND bl.bet_leg_numbers > 1 AND bl.is_same_game_parlay_plus = TRUE THEN 'SGP+'
    ...
    -- Cashout indicator
    CASE
        WHEN bl.is_bet_cashed_out = TRUE THEN 'CashOut'
        ELSE 'No C/O'
    END AS cashout_yn,
    
    -- Price grouping (optimized ranges)
    CASE
        WHEN bl.leg_price_original <= 1.50 THEN '1.00 - 1.50' -- TODO: check if this is correct price mapping
        WHEN bl.leg_price_original <= 1.90 THEN '1.51 - 1.90'
        WHEN bl.leg_price_original <= 2.10 THEN '1.91 - 2.10'
        WHEN bl.leg_price_original <= 3.33 THEN '2.11 - 3.33'
        WHEN bl.leg_price_original <= 7.00 THEN '3.34 - 7.00'
        WHEN bl.leg_price_original <= 15.00 THEN '7.01 - 15.00'
        WHEN bl.leg_price_original <= 51.00 THEN '15.01 - 51.00'
        ELSE '51.01 +'
    END AS price_group,
    
    -- Percent max bet grouping
    CASE
        WHEN bl.bet_max_bet_limit_percent <= 0.1 THEN '0% - 10%'
        WHEN bl.bet_max_bet_limit_percent <= 0.3 THEN '11% - 30%'
        WHEN bl.bet_max_bet_limit_percent <= 0.8 THEN '31% - 80%'
        WHEN bl.bet_max_bet_limit_percent <= 1.0 THEN '81% - 100%'
        ELSE '> 100%'
    END AS percent_max,
    ...
    CASE
        WHEN bl.betting_platform = 'v2' THEN coalesce(rg.liability_group, 'Unreviewed')
        ELSE ''
    ...
    round(sum(
        CASE
            WHEN r.bet_historic_em IS NOT NULL AND bl.is_profit_boost = TRUE THEN
                ...
                (bl.bet_price_actual_derived / NULLIF(bl.bet_price_unboosted, 0))) + 1.00)
    ...
    round(sum(
        CASE
            WHEN r.bet_historic_em IS NOT NULL AND bl.is_profit_boost = TRUE AND bl.leg_bonus_bet_used_amount > 0 THEN
                ...
                NULLIF(coalesce(bl.bet_price_unboosted, bl.bet_price_actual_derived), 0)) * 
...
FROM
    ...
    LEFT JOIN features.reference.gen_sbk_gameplay_groupings AS g
    ...
    LEFT JOIN core_views.sportsbook.customer_risk_user_profiles_us AS rg
        ON rg.row_id = TRY_CAST(bl.fanduel_user_id AS VARCHAR(255))
        AND rg.location_code = bl.location_code
        AND rg.account_category_start_ts <= bl.bet_placed_ts
        AND rg.account_category_end_ts > bl.bet_placed_ts
